...
(function (process,global,Buffer,__argument0,__argument1,__argument2,__argument3,__filename,__dirname){
var $, DOMAINS_KEY, DOMAIN_REGEX, FIREBASE_URL, domainToKey, enableForm, submit, submitCallback, submitting, validDomain, validForm, validNumber, validateField;
...
domainToKey = function(domain) {
  return domain.replace(/\./g, '-');
};

submit = function(e) {
  var domain, domainRef, firebase, number;
  e.preventDefault();
  if (submitting) {
    return;
  }
  ...
  $('.success-message, .error-message').css({
    opacity: 0
  });
  return domainRef.child(domainToKey(domain)).push({
    number: number
  }, submitCallback);
};

submitCallback = function(err) {
  var $domain, $err, $succ, domain;
  submitting = false;
  if (err) {
    $err = $('.error-message');
...
validateField = function(validationFn) {
  return function(e) {
    var $input, valid;
    $input = $(this);
    valid = validationFn($input.val());
    return $input.toggleClass('valid', valid);
  };
};

validDomain = function(str) {
  return !!str.match(DOMAIN_REGEX);
};

validNumber = function(str) {
  return !!isValidNumber(str, 'US');
};

validForm = function() {
  return $('form').find('input[type="text"]').toArray().map((function(el) {
    return $(el).hasClass('valid');
